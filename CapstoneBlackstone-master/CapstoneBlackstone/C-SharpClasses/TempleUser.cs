...
namespace CapstoneBlackstone.C_SharpClasses
{
    ...
    public class College
    {
        ...
        {
            private static string webServiceUsername = ConfigurationManager.ConnectionStrings["Connection_WS_Username"].ToString(); //WSInfo.webServiceUsername;
            private static string webServicePassword = ConfigurationManager.ConnectionStrings["Connection_WS_Password"].ToString(); //WSInfo.webServicePassword;
            ...
            public static string[] getAllMajors()
            {
                try
                {
                    //string username = ConfigurationManager.ConnectionStrings["Connection_WS_Username"].ToString();
                    //string password = ConfigurationManager.ConnectionStrings["Connection_WS_Password"].ToString();

                    CapstoneBlackstone.WS_ProgramsAndDegrees.ProgramsAndDegrees programsProxy = new CapstoneBlackstone.WS_ProgramsAndDegrees.ProgramsAndDegrees();
                    CapstoneBlackstone.WS_ProgramsAndDegrees.ProgramsAndDegreesEntry[] results = new CapstoneBlackstone.WS_ProgramsAndDegrees.ProgramsAndDegreesEntry[30];
                    results = programsProxy.GetAllPrograms(webServiceUsername, webServicePassword);

                    string[] majors = new string[results.Length];
                    string major;
                    for (int i = 0; i < results.Length; i++)
                    {
                        major = results[i].major.ToString();
                        majors[i] = major;
                    }
